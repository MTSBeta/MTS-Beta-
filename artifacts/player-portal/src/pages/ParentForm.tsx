...
import { useGetPlayerByParentCode, useSubmitParentResponses } from "@workspace/api-client-react";

export default function ParentForm() {
  const [match, params] = useRoute("/parent/:code");
  ...
  const { data: player, isLoading, isError } = useGetPlayerByParentCode(code, {
    query: { retry: false }
  });

  const submitMutation = useSubmitParentResponses();
  ...
  const handleSubmit = async () => {
    ...
    const formattedResponses = PARENT_QUESTIONS.map((q, idx) => ({
      ...
      <Layout hideLogo>
        ...
        >
          ...
          <p className="text-white/70 text-lg">
            Your perspective has been successfully added to {player.playerName}'s development profile.
          </p>
        </motion.div>
      </Layout>
    );
      ...
      <div className="w-full max-w-3xl mx-auto flex flex-col pt-8 pb-12">
        <div className="text-center mb-10">
          ...
          <p className="text-lg text-white/70 font-medium">
            Contributing to the profile of <span className="text-white font-bold">{player.playerName}</span> • {player.academyName}
          </p>
        </div>
        ...
        <div className="glass-panel p-6 md:p-10 rounded-3xl space-y-8">
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 mb-8">
            <p className="text-white/80 italic text-sm md:text-base leading-relaxed">
              Your insights are crucial to building a complete picture of the player's development, resilience, and character. Please answer the following questions as openly as possible.
            </p>
          </div>
