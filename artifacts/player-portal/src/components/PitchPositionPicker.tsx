...
export function PitchPositionPicker({ value, onChange, error }: Props) {
        ...
        <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 overflow-hidden">
          <div className="flex items-center gap-3 px-4 pt-3 pb-2">
            <div className="w-10 h-10 rounded-full bg-amber-500/20 border border-amber-500/50 flex items-center justify-center flex-shrink-0">
              <span className="text-amber-400 font-black text-xs font-display">{selectedPos.id}</span>
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-white font-bold text-sm">{selectedPos.displayName}</div>
              <div className="text-amber-400 text-xs font-semibold uppercase tracking-wide">{selectedPos.archetype}</div>
            </div>
            <button
              type="button"
              onClick={() => onChange("")}
          ...
          <div className="px-4 pb-3 border-t border-amber-500/20 pt-2 space-y-1">
